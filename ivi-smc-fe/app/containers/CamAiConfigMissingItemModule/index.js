...
import makeSelectCamAiConfigMissingItemModule from './selectors';
...
import { CAMERA_AI_API_SRC, CAM_AI_CONFIG_MISSING_ITEM_MODULE } from '../apiUrl';
import { showError, showSuccess } from '../../utils/toast-utils';
// import { FAKE_DATA } from './constants';
import PopupDelete from '../../components/Custom/popup/PopupDelete';

const key = 'camAiConfigMissingItemModule';

const typeProcess = 'vsmMissingItem';
...
const initValues = {
  ...
  minioBucket: null,
  kafka: null,
  kafkaTopic: 'UAT_TNP_MISSING_ITEM_REQUEST',
...
export function CamAiConfigMissingItemModule() {
  useInjectReducer({ key, reducer });
  useInjectSaga({ key, saga });
  const { missingItemId, edgeId } = useParams();
  ...
  const schema = yup.object({
    ...
    kafkaTopic: !rawMode
      ? yup
          ...
          .required('Trường này bắt buộc nhập')
      : false,
    cam_infor: !rawMode ? yup.array().min(1, 'Please add camera') : false,
    devconfig: yup
      .string()
      .trim()
      .nullable()
  ...
  const handleAddCam = val => {
    ...
    other.camera.event_missingitems = 'missingitems';
    other.camera.list_items = ['backpack', 'handbag', 'suitcase'];
    other.camera.time_wait = 200;
    other.camera.stop_time = 10;
  ...
  const fetchFileVersions = async () => {
    ...
    try {
      const res = await getApi(
        CAM_AI_CONFIG_MISSING_ITEM_MODULE.GET_FILE_VERSIONS(edgeId),
      );
  ...
  const fetchMinioBucket = async id => {
    ...
    try {
      const res = await getApi(
        CAM_AI_CONFIG_MISSING_ITEM_MODULE.GET_MINIOS(id),
      );
  ...
  const fetchDetail = async () => {
    try {
      const res = await getApi(
        CAM_AI_CONFIG_MISSING_ITEM_MODULE.GET_FPGA_PROCESS(missingItemId),
      );
      return res.data;
    } catch (error) {
      showError(error);
  ...
  const handleCreate = value => {
    setLoading(true);
    postApi(CAM_AI_CONFIG_MISSING_ITEM_MODULE.ADD_FPGA_PROCESS, value)
      .then(() => {
  ...
  const handleUpdate = value => {
    setLoading(true);
    putApi(
      CAM_AI_CONFIG_MISSING_ITEM_MODULE.UPDATE_FPGA_PROCESS(missingItemId),
      value,
    )
      .then(() => {
  ...
  return (
    <React.Fragment>
      <Helmet>
        <title>Missing Item Module - vsmMissingItem</title>
        <meta name="description" content="Missing Item Module" />
      </Helmet>
      {loading && <Loading />}
      <PageHeader
        title={
          missingItemId
            ? `Edit process: 'EDGE_MissingItem'`
            : 'Create new process'
        }
        showBackButton
        onBack={() => {
              ...
              <React.Fragment>
                ...
                <Grid item container spacing={3}>
                  <Grid item container spacing={3}>
                    <Grid item xs={4}>
                      <p className={classes.label}>
                        Kafka server <span style={{ color: 'red' }}>*</span>
                      </p>
                      <CustomSelect
                        control={control}
                        errors={errors}
...
`;
CamAiConfigMissingItemModule.propTypes = {};

const mapStateToProps = createStructuredSelector({
  camAiConfigMissingItemModule: makeSelectCamAiConfigMissingItemModule(),
});
...
export default compose(withConnect)(CamAiConfigMissingItemModule);
