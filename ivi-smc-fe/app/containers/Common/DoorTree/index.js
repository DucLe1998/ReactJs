...
import { AC_GROUP_DOOR_API } from 'containers/apiUrl';
import { useIntl } from 'react-intl';

// selectionMode="multiple" || "single"
export function DoorTree({ value, onValueChanged, selectionMode = 'single' }) {
  const intl = useIntl();
      ...
      createChildren={async parent => {
        ...
        try {
          const response = await callApi(
            `${AC_GROUP_DOOR_API.GET_CHILD}?parentId=${parentId}`,
            METHODS.GET,
          );
