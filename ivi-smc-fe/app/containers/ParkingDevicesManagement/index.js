...
export function ParkingDevicesManagement({ location }) {
  ...
  useEffect(() => {
    ...
    if (state.pkLot || state.entrypt || state.lane) {
      filterUpdate.pkLot = state.pkLot;
      filterUpdate.entrypt = state.entrypt;
      filterUpdate.lane = state.lane;
  ...
  const fetchDataSource = async () => {
    ...
    const params = {
      ...
      laneId: filter?.lane?.id || '',
      deviceTypeId: filter?.deviceType?.id || '',
      limit: filter.limit,
      page: filter.page,
    };
    await getApi(`${API_PARKING_LOT.PARKING_DEVICES}`, _.pickBy(params))
      .then((response) => {
  ...
  const handlerDelete = () => {
    ...
    setLoading(true);
    axios
      .delete(`${API_PARKING_LOT.PARKING_DEVICES}/${deleteId}`)
      .then(() => {
        setDeleteid(null);
  ...
  const columns = [
    ...
    },
    {
      ...
      cellRender:({data}) => {
        if(data?.fileId){
          return (
            <a href={API_FILE.DOWNLOAD_PUBLIC_FILE(data.fileId)}
              target="_blank"
              download
            >
              {data.swVersionName}
    ...
    <>
      ...
      <>
        ...
        <PageSearch
          ...
          placeholderSearch="Nhập tên thiết bị, IP..."
          ...
          <Link
            to={{
              ...
              state: {
                ...
                lane: { id: state?.lane?.id, name: state?.lane?.name },
              },
            }}
          >
            Quản lý làn xe
