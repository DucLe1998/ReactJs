...
export default function DialogEditGuest({
  ...
  const [guestLocalId, setGuestLocalId] = useState();
  const {
    handleSubmit,
    control,
    reset,
  ...
  const onSubmitForm = (values) => {
    const result = { ...values };
    //result.guestLocalId = +guestLocalId;
    setWarning(false);
    clearErrors();
    onSuccess(result);
  };
  return (
    ...
    >
      <form onSubmit={handleSubmit(onSubmitForm)}>
        <DialogTitle>{intl.formatMessage(messages.addGuest)}</DialogTitle>
        <DialogContent style={{ overflowY: 'unset' }}>
          <Grid container spacing={3}>
                    ...
                    render={(props) => (
                      <TextField
                        ...
                        onChange={(e) => {
                          ...
                          //setGuestLocalId(e.target.value);
                        }}
                      />
                    )}
                  />
            ...
            <Grid item xs={6}>
              <Grid container direction="column">
                <p className={classes.label}>Địa chỉ công ty</p>
                <Controller
                  control={control}
                  name="addressCompany"
                  render={(props) => (
