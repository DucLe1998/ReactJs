...
import { REPEAT_TYPE_MAP, STATUS_APPROVE_MAP } from '../modules';
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} md={6}>
            <FormControl size="small" fullWidth margin="dense">
              ...
              <MultiAutocomplete
                name="approvalStatusMe"
                value={data.approvalStatusMe}
                placeholder="Trạng thái"
                options={Object.values(STATUS_APPROVE_MAP)}
                getOptionLabel={(option) => option?.text || ''}
                getOptionSelected={(option, selected) =>
                  option.key === selected.key
                }
                onChange={(e, newValue) => {
                  handleFilterChange('approvalStatusMe', newValue);
                }}
              />
            </FormControl>
          </Grid>
